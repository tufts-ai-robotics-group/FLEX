...
class SuccessProjectionCallBack(BaseCallback):
    def __init__(self, log_interval=100000, verbose=0):
...
def train(run_id):
    ...
    pris_force_policy_dir += '/' + str(n_runs_pris+1)
    rev_force_policy_dir += '/' + str(n_runs_rev+1)
    ...
    all_curriculum_rewards = []
    all_curriculum_projections = []
    ...
    for curriculum_idx, current_curriculum in enumerate(curriculas):
        ...
        cur_curriculum_timesteps = episodes_schedule[curriculum_idx] * max_timesteps
        ...
        is_prismatic = 'train-drawer-1' in cur_curriculum_objects
        curriculum_env_kwargs['env_name'] = 'MultipleRevoluteEnv' if not is_prismatic else 'TrainPrismaticEnv' 
        n_training_steps = n_pris_steps if is_prismatic else n_rev_steps
        policy_name = f'force_td3_{n_training_steps}' if is_prismatic else f'force_td3_revolute_{n_training_steps}'

        env_kwargs_list = [
            curriculum_env_kwargs for i in range(n_envs)
        ]
        for i in range(n_envs):
            env_kwargs_list[i]['object_name'] = random.choice(cur_curriculum_objects) if not isinstance(cur_curriculum_objects, str) else cur_curriculum_objects
            # print(f'env {i} is training with object {env_kwargs_list[i]["object_name"]}')
        ...
        policy = sb3.TD3('MlpPolicy', env, action_noise=action_noise, verbose=1)
        checkpoint_dir = pris_force_policy_dir if is_prismatic else rev_force_policy_dir 
        # n_curris = sum(os.path.isdir(os.path.join(checkpoint_dir, entry)) for entry in os.listdir(checkpoint_dir))
        policy = policy.load(checkpoint_dir+'/'+policy_name) if n_training_steps > 0 else policy # load the policy if it exists
        policy.set_env(env)
        new_policy_name = f'force_td3_{n_training_steps+cur_curriculum_timesteps}' if is_prismatic else f'force_td3_revolute_{n_training_steps+cur_curriculum_timesteps}'
        n_pris_steps += cur_curriculum_timesteps if is_prismatic else 0 
        n_rev_steps += cur_curriculum_timesteps if not is_prismatic else 0
        policy.learn(total_timesteps=cur_curriculum_timesteps, log_interval=1, tb_log_name=new_policy_name)

        curriculum_end_time = time.time()
        policy.save(checkpoint_dir+'/'+new_policy_name)
        if curriculum_idx == 0:
            policy.save(checkpoint_dir+'/prismatic_final') 
        elif curriculum_idx == len(curriculas) - 1:
            policy.save(checkpoint_dir+'/revolute_final')
            
        if is_prismatic:
            prismatic_training_time += curriculum_end_time - curriculum_start_time
        else:
            revolute_training_time += curriculum_end_time - curriculum_start_time
        env.close()
    train_end_time = time.time() 
    print('total training time: ', train_end_time - train_start_time)
    time_dict = {
        'total_training_time': train_end_time - train_start_time,
        'prismatic_training_time': prismatic_training_time,
        'revolute_training_time': revolute_training_time
    }
    with open(f'{pris_force_policy_dir}/training_time.json', 'w') as f:
        json.dump(time_dict, f)
    with open(f'{rev_force_policy_dir}/training_time.json', 'w') as f:
        json.dump(time_dict, f)
    ...
    #         ]
    #         for i in range(n_envs):
    #             env_kwarg_list[i]['object_name'] = current_episode_object = random.choice(cur_curriculum_objects) if not isinstance(cur_curriculum_objects, str) else cur_curriculum_objects
    #         env = SubprocVecEnv([make_env(curriculum_env_kwargs) for i in range(n_envs)])
    ...
    #         policy = sb3.TD3('MlpPolicy', env, verbose=1)
