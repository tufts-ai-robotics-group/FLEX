...
def train(run_id):
    ...
    while prismatic_training_timesteps < 6000 * 200:
        ...
        if prismatic_training_timesteps % 40 == 0:
            policy.update(replay_buffer, 30, batch_size, gamma, polyak, policy_noise_schedule[prismatic_noise_idx], noise_clip, policy_delay)
        if prismatic_training_timesteps % 100000 == 0:
        ...
        if prismatic_training_timesteps > 1000 * 200 and np.mean(prismatic_projections[-100]) > 0.9:
    ...
    policy.save(pris_force_policy_dir, 'prismatic_final')
    prismatic_env.close()
    json_dict = {
    ...
    for curriculum_idx, current_curriculum in enumerate(curriculas):
        ...
        while cur_curriculum_training_steps < episodes_schedule[curriculum_idx] * 200:
            ...
            if cur_curriculum_training_steps > 200 * 200 and np.mean(cur_curriculum_projections[-100]) > 0.85:
